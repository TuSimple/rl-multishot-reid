...
eps = 1e-10 + 1e-5
bn_mom = 0.9
#fix_gamma_flag = False


def ConvFactory(data, num_filter, kernel, stride=(1,1), pad=(0, 0), name=None, suffix=''):
    conv = mx.symbol.Convolution(data=data, num_filter=num_filter, kernel=kernel, stride=stride, pad=pad, name='conv_%s%s' %(name, suffix))
    bn = mx.symbol.BatchNorm(data=conv, fix_gamma=fix_gamma_flag, eps=eps, momentum=bn_mom, use_global_stats=global_stats_flag, name='bn_%s%s' %(name, suffix))
...
def get_symbol(data, num_classes=1000, fix_gamma=False, global_stats=False):
    global fix_gamma_flag, global_stats_flag
    fix_gamma_flag, global_stats_flag = fix_gamma, global_stats
    # data
    ...
    pool2 = mx.symbol.BlockGrad(pool2, name='block_stage2')
    # stage 2
    ...
    #in5b = mx.symbol.BlockGrad(in5b, name='block_in5b')
    # global avg pooling
