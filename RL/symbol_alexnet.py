...
def get_symbol(data=None, params=None, fix_gamma=None, global_stats=None):
    if params is None:
    ...
    x = mx.symbol.BlockGrad(x, name='block_conv2')
    ...
    x = mx.sym.Activation(data=x, act_type='relu')
    #x = mx.symbol.BlockGrad(x, name='block_conv3')
    ...
    x = mx.sym.Activation(data=x, act_type='relu')
    #x = mx.symbol.BlockGrad(x, name='block_conv4')
    ...
    x = mx.sym.Activation(data=x, act_type='relu')
    x = mx.sym.Pooling(data=x, kernel=(3, 3), stride=(2, 2), pool_type='max')
    #x = mx.symbol.BlockGrad(x, name='block_conv5')
